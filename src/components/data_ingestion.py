...
class DataIngestion:
    ...
    def export_data_into_raw_data_dir(self)->pd.DataFrame:
        ...
        """
        try:
            logging.info(f"Exporting data from mongodb or CSV fallback")
            ...
            if mongo_url and mongo_url.strip():
                ...
                # CSV fallback
                csv_path = os.path.join("notebooks","Data","jfk_weather_cleaned.csv")
                if not os.path.exists(csv_path):
                    raise Exception(f"CSV fallback not found at {csv_path}")
                df = pd.read_csv(csv_path)
                # Save to raw_data subdirectory as expected by validation
                raw_data_path = os.path.join(raw_batch_files_path, 'raw_data')
                os.makedirs(raw_data_path, exist_ok=True)
                feature_store_file_path = os.path.join(raw_data_path, 'visibility_20240120_120000.csv')
                df.to_csv(feature_store_file_path, index=False)
                logging.info(f"CSV fallback copied to {feature_store_file_path} with shape {df.shape}")
    ...
    def initiate_data_ingestion(self) -> Path:
        ...
        try:
            ...
            logging.info("Got the data from mongodb/CSV fallback")
            ...
            return os.path.join(self.data_ingestion_config.data_ingestion_dir, 'raw_data')
